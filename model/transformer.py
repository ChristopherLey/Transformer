...
class Head(nn.Module):
    ...
    def __init__(
        self, input_size: int, head_size: int, block_size: int, dropout: float = 0.5
    ):
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.heads = nn.ModuleList(
            [
                Head(input_size, head_size, block_size, dropout=dropout)
...
class Block(nn.Module):
    ...
    def __init__(
        self, input_size: int, block_size: int, num_heads: int, dropout: float = 0.5
    ):
        super(Block, self).__init__()
        self.norm1 = nn.LayerNorm(input_size)  # per token normalisation
        self.self_attention = MultiHeadAttention(
            input_size, block_size, num_heads, dropout=dropout
        )
        self.norm2 = nn.LayerNorm(input_size)
        self.feed_forward = FeedForward(
            input_size, hidden_dim=input_size * 4, dropout=dropout
        )
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(*block_set)
    ...
    def forward(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None):
        ...
        )  # broadcasting will copy positional embedding along the batches
        x = self.blocks(x)
